...
def all_deps():
    ...
    local('pip install -r pip-dependencies')
    local('pip install -r build-dependencies')
...
env.project = 'greatwar'
env.rev_tag = ''
env.git_rev = ''
env.remote_path = '/home/httpd/sites/greatwar'
env.url_prefix = 'greatwar'
env.remote_proxy = None
env.remote_acct = 'greatwar'

def configure(path=None, user=None, url_prefix=None, remote_proxy=None):
    'Configuration settings used internally for the build.'

    env.version = greatwar.__version__
    config_from_git()
...
def config_from_git():
    ...
    if greatwar.__version_info__[-1]:
...
def deploy(path=None, user=None, url_prefix='', remote_proxy=None,
    ...
    Example usage:
      fab deploy:/home/greatwar/,gw -H servername
